...
A tiny todo app that demonstrates the two halves of KRIY on one shared data store:
...
## 3. Connect it to KRIY

1. In KRIY, go to **MCP Connections** and add a connection pointing at
...
This shows the **event-driven, multi-tenant** side of KRIY. The app does one thing when
a todo is completed: it **emits an event**. It has no idea what should happen — that's a
*workflow*, defined per user in KRIY. Different users can react to the same event in
completely different ways.

How the app side works (`app.py`) — note there's no agent id and no rule here:

```python
from kriy_agentic import KriyClient
kriy = KriyClient(api_key=os.environ["KRIY_API_KEY"])  # identifies the user
...
hiccup never breaks the app. KRIY looks up every enabled workflow you have for
...
curl -X POST http://localhost:8000/api/v1/workflows \
  -H 'Content-Type: application/json' -H "X-API-Key: kriy-..." \
  -d '{
...
2. Generate a **per-user API key** (Config → API key; starts with `kriy-`). Use that — *not*
...
4. `cp .env.example .env`, set `KRIY_API_KEY`, restart the app, complete every todo, and
...
> same event, the app can't own the logic. It just reports *what happened*; KRIY owns
